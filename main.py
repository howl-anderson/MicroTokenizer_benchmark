...
from evaluate_with_tool import evaluate_by_files
...
def get_train_data_file(corpus_name="msr"):
    ...
    if corpus_name == 'pd':
        return os.path.join(current_dir_path, "data/people_daily/training.utf8")
    else:
...
def get_input_data_file(corpus_name="msr"):
    ...
    if corpus_name == 'pd':
        return os.path.join(current_dir_path, "data/people_daily/test.utf8")
    else:
...
def test_tokenizer_on_corpus(corpus, tokenizer):
    ...
    score = evaluate_by_files(corpus, tokenizer)
...
def benchmark_sequence_test_performance():
    # TODO: seems not do the test work but only evaluate it
    all_corpus_list = corpus_registry.values()
    all_tokenizer_list = tokenizer_registry.keys()
